...
#                 connection.rollback()
#                 print(f"A UniqueViolation error occurred: {e}. Trying row-by-row to find duplicates.")
                
#                 # Fallback to row-by-row insertion to log failed entries
#                 try:
...
#                             ON CONFLICT ({', '.join(f'"{col}"' for col in conflict_target)}) DO NOTHING;
#                             """
#                             cur.execute(single_insert_query, row)
#                             connection.commit()
#                         except psycopg2.errors.UniqueViolation:
#                             connection.rollback()
#                             failed_entries.append((row.year, row.grand_prix))
#                         except Exception as row_e:
#                             connection.rollback()
#                             print(f"An unexpected error occurred for row: {row}. Error: {row_e}")
#                             failed_entries.append((row.year, row.grand_prix))
...
#             # finally:
#             #     if failed_entries:
#             #         print("\nThe following (year, grand_prix) entries failed the upsert:")
#             #         for year, gp in failed_entries:
#             #             print(f" - Year: {year}, Grand Prix: {gp}")
#     print("\n The following (year, grand_prix) entries failed the upsert:")
#     for year, gp in failed_entries:
#         print(f"- Year: {year}, Grand Prix: {gp}")
